	...
	timer.Simple(1, function()
		if not VJBASE_INSTALLED and not VJBASE_ERROR_MISSING then
			...
			if CLIENT then
				...
				label1:SetText("Hey dummy, it seems here that ya don't got VJ Base installed on ya Gmod.")
				label1:SizeToContents()
				
				local label2 = vgui.Create("DLabel", frame)
				label2:SetPos(10, 70)
				label2:SetText("Ya see... if y'all want this addon workin' then I suggest ya get VJ Base installed on this here server usin' the link below.\n                                                 Then ya better make sure that it's enabled then restart yer game.")
			...
			elseif SERVER then
				...
				timer.Create("VJBASE_ERROR_MISSING", 50, 0, function()
