...
function ENT:OnThink()
	...
	if self.BreederAttackMode == 0 && !self:IsBusy() && self.BreederHeadcrabCount > 0 && CurTime() > self.Breeder_NextPullHeadcrabTime && ( (self.VJ_IsBeingControlled == false && IsValid(self:GetEnemy()) && (self:GetPos():Distance(self:GetEnemy():GetPos()) < 500) ) or (IsValid(self.VJ_TheController) && self.VJ_TheController:KeyDown(IN_RELOAD))) then
		--make sure we're available to pull out a crab, or if the player controller pressed R
		self:Breeder_PulloutHeadcrab()
	end
	
	if self.BreederAttackMode == 0 && !self:IsBusy(Activities) && self.BreederBabycrabCount == 0 && self.BreederMaxBabycrabCount > 0 && CurTime() > self.Breeder_NextBabycrabTime && ( (self.VJ_IsBeingControlled == false && IsValid(self:GetEnemy()) && (self:GetPos():Distance(self:GetEnemy():GetPos()) < 950) ) or (IsValid(self.VJ_TheController) && self.VJ_TheController:KeyDown(IN_DUCK))) then
